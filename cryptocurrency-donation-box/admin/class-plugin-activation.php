    ...
    final class CDDBCActivation
    {
        ...
        public function add_admin_menu($context)
        {
            ...
            if (!get_option('cdbbc_activated')) {
                $this->hook_name = add_submenu_page('cdbbc-crypto-donations', __('Plugin Activation', 'cryptocurrency-donation-box'), __('Plugin Activation', 'cryptocurrency-donation-box'), 'manage_options', 'cdbbc-activation', array($this, 'render'));
            }
        }
            ...
            if ($run_setup) {
                if (delete_transient('cdbbc_init_activation')) {
                    try {
                        ...
                        if (WP_DEBUG) { 
                            error_log('CDBBC Setup Error: ' . $e->getMessage()); // Only logs when debugging is enabled
                        }
                    }
    ...
    ?>
        <div class="wrap cdbbc-activation-page">
            <div class="card-top">
                <img class="img" src="<?php echo esc_url(CDBBC_URI . 'assets/images/cryptodonation-logo.png'); ?>" alt="Logo">
                <p id="messager" class="description"><?php echo esc_html__('One more minute, please accept our Terms & Conditions!', 'cryptocurrency-donation-box'); ?></p>
                ...
                <form method="POST" action="">
                    ...
                    <label>
                        ...
                        <span>
                            <?php 
                            // translators: %s represents the link to Terms & Conditions and Privacy Policy
                            echo sprintf(
                                __(
                                    'By registering your site with adastracrypto.com(Plugin Author), you are agreeing to these %sTerms & Conditions%s and %sprivacy policy%s.',
                                    'cryptocurrency-donation-box'
                                ),
                                '<a href="' . admin_url() . 'admin.php?page=cdbbc-crypto-donations#tab=terms-conditions" target="_blank">',
                                '</a>',
                                '<a href="' . admin_url() . 'admin.php?page=cdbbc-crypto-donations#tab=terms-conditions" target="_blank">',
                                '</a>'
                            ); ?>
                    ...
                    <div class="card-bottom">
                        <button id="meta-plugin-activate-btn" class="button button-primary" type="submit" data-plugin="cdbbc">
                            <?php echo __('Activate', 'cryptocurrency-donation-box'); ?>
                        </button>
                        <a class="to-dashboard" href="<?php echo esc_url(admin_url()); ?>">&larr; <?php echo esc_html__('Back to dashboard', 'cryptocurrency-donation-box'); ?></a>
                    </div>
                </form>
                <p class="permalink">
        ...
        public function enqueue_assets($hook_name)
        {
            ...
            wp_localize_script(
                ...
                array(
                    ...
                    'pluginUri' => CDBBC_URI,
                    'tosRequired' => __('You must accept our Terms & Conditions!', 'cryptocurrency-donation-box')
                )
            );
